...
Syntax.register('clang', function(brush) {
    ...
    brush.push({pattern:/@property\((.*)\)[^;]+;/gmi,klass:'objective-c-property',allow:'*'});
    var propertyAttributes = ["getter","setter","readwrite","readonly","assign","retain","copy","nonatomic"];
    brush.push(propertyAttributes, {klass:'keyword',only:['objective-c-property']})
    brush.push({pattern:/@(?=")/g,klass:'string'});
    brush.push(Syntax.lib.camelCaseType);
    brush.push({pattern:/#.*$/gmi,klass:'preprocessor',allow:['string']});
    brush.push(Syntax.lib.cStyleComment);
    brush.push(Syntax.lib.cppStyleComment);
    brush.push(Syntax.lib.webLink);
    brush.push({pattern:/\w+:(?=.*(\]|;|\{))(?!:)/g,klass:'function'});
    brush.push({pattern:/[^:\[]\s+(\w+)(?=\])/g,matches:Syntax.extractMatches({klass:'function'})});
    brush.push({pattern:/-\s*(\([^\)]+?\))?\s*(\w+)\s*\{/g,matches:Syntax.extractMatches({index:2,klass:'function'})});
