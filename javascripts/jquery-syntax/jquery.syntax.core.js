...
Syntax.modeLineOptions = {'tab-width':function(name, value, options) {
    options.tabWidth = parseInt(value, 10);
}};
...
Syntax.lib.webLinkProcess = function(queryURI, lucky) {
    ...
    return function(element, match) {
        return jQuery('<a>').attr('href', queryURI + encodeURIComponent(element.text())).attr('class', element.attr('class')).append(element.contents());
    };
};
...
Syntax.Brush.prototype.derives = function(name) {
    ...
    this.rules.push({apply:function(text, expr, offset) {
